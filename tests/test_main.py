...
class TestMainSignalHandlers(unittest.TestCase):
    ...
    def test_main_registers_signal_handlers(self, mock_signal, mock_start, mock_health, mock_time):
    ...
    @patch(f"{MAIN_MODULE}.start_health_server")
    @patch(f"{MAIN_MODULE}.start_scheduler")
    def test_main_starts_health_server_before_scheduler(self, mock_start, mock_health, mock_signal, mock_time):
        mock_time.sleep.side_effect = KeyboardInterrupt
        from flight_watcher.__main__ import main
        call_order = []
    ...
    def test_signal_handler_full_shutdown_sequence(self, mock_sys, mock_get_state, mock_stop_sched, mock_stop_health, mock_dispose):
