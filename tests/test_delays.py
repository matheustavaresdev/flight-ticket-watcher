...
def test_random_delay_sleeps_within_range():
    with patch("flight_watcher.delays.time.sleep") as mock_sleep, \
...
def test_random_delay_uses_custom_range():
    ...
    with patch("flight_watcher.delays.time.sleep"), \
...
def test_random_delay_uses_env_defaults():
    with patch("flight_watcher.delays.time.sleep"), \
         patch("flight_watcher.delays.random.uniform", return_value=10.0) as mock_uniform, \
